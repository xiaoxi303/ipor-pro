...
export default function Home() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
        ...
        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-8 space-y-12">
              ...
              >
                <InfoCard title="地理位置信息" icon={Globe}>
                  ...
                  <InfoItem label="ASN" value={data?.asn ? `AS${data.asn}` : "Unknown"} />
                  <InfoItem label="运营商" value={data?.isp} highlight />
                </InfoCard>

                <InfoCard title="通讯与环境" icon={Smartphone}>
                  <InfoItem label="移动运营商" value={data?.mobile_brand || "非移动网络"} />
                  <InfoItem label="MCC/MNC" value={data?.mcc && data?.mnc ? `${data.mcc}/${data.mnc}` : "N/A"} />
                  <InfoItem label="经纬度" value={`${data?.latitude}, ${data?.longitude}`} />
                  <InfoItem label="气象站" value={data?.weather_station_name || "未知"} />
              ...
              <motion.div 
                ...
                className="rounded-[3rem] overflow-hidden border border-white/10 shadow-2xl h-[500px]"
              >
                <Map latitude={data?.latitude || 0} longitude={data?.longitude || 0} />
              </motion.div>
              
              <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }}>
                <BgpDeepAnalysis asn={data?.asn} asnOrg={data?.asn_org} ip={data?.ip} />
              </motion.div>
            ...
            <div className="lg:col-span-4 space-y-8">
              ...
              <motion.div variants={{ hidden: { opacity: 0, x: 20 }, visible: { opacity: 1, x: 0 } }}>
                <SecurityInsights riskScore={data?.riskScore} isProxy={data?.is_proxy} isp={data?.isp} />
              </motion.div>

              <motion.div variants={{ hidden: { opacity: 0, x: 20 }, visible: { opacity: 1, x: 0 } }}>
                <IpDatabaseVerification riskScore={data?.riskScore} isProxy={data?.is_proxy} isp={data?.isp} ip={data?.ip} usageType={data?.usage_type} />
              </motion.div>
      ...
      <footer className="border-t border-white/10 py-12 text-center text-muted-foreground text-xs uppercase tracking-widest font-black">
        <div className="container mx-auto px-4">
          <p>© 2026 IPor. Powered by IP2Location & IPPure Core Intelligence</p>
